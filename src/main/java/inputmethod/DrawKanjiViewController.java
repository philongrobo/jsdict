...
import radicalsearch.AbstractActivity;
import radicalsearch.AedictApp;
import radicalsearch.KanjiAnalyzeActivity;
import radicalsearch.Radicals;
...
@SuppressLint("NewApi")
public class DrawKanjiViewController extends AbstractActivity {
	/**
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		fullStringRadical = getIntent().getStringExtra("stringRadical");
		final TextView tv = new TextView(this);
		tv.setTextSize(FONT_SIZE_DIP);
		radicalViewSizePixels = (int) tv.getPaint().getFontSpacing();
		final DisplayMetrics dm = new DisplayMetrics();
		getWindowManager().getDefaultDisplay().getMetrics(dm);
		radicalsPerRow = dm.widthPixels
				/ (radicalViewSizePixels + 2 * PADDING_PIXELS);
		currentColumn = -1;
		row = null;
		int strokeCount = -1;
		...
		findViewById(R.id.btnRadicalsSearch).setOnClickListener(
				AndroidUtils.safe(this, new View.OnClickListener() {

					public void onClick(View v) {
						performSearch();
					}
				}));
		this.setTitle(getString(R.string.kanjiLookup) + ": ");
	}

	/**
	 * The real size of the font character. Computed from
	...
	private void addRadicalToggle(final TableLayout v, final Character radical,
			final int strokes) {
		...
		String drawable = radical != null ? Radicals.getRadical(radical).resource
				: "";
		final View vv;
		if (drawable != "") {
			final TextView tv = new TextView(this);
			vv = tv;
			tv.setText(radical == null ? String.valueOf(strokes) : drawable
					.toString());
			...
			final TextView tv = new TextView(this);
			vv = tv;
			tv.setText(radical == null ? String.valueOf(strokes) : radical
					.toString());
			tv.setGravity(Gravity.CENTER);
			tv.setTextSize(FONT_SIZE_DIP);
			tv.setHeight(radicalViewSizePixels + 2 * PADDING_PIXELS);
		...
		vv.setPadding(PADDING_PIXELS, PADDING_PIXELS, PADDING_PIXELS,
				PADDING_PIXELS);
		if (radical != null) {
	...
	private String getRadicals() {
		...
		for (int i = 0; i < v.getChildCount(); i++) {
			...
			for (int j = 0; j < tr.getChildCount(); j++) {
				final PushButtonListener pbl = (PushButtonListener) tr
						.getChildAt(j).getTag();
	...
	private Integer getInt(final int editResId) {
		final String text = ((EditText) findViewById(editResId)).getText()
				.toString();
		try {
	...
	private void performSearch() {
		...
		if (radicals.length() == 0) {
			new DialogUtils(this)
					.showErrorDialog(R.string.no_radicals_selected);
			return;
		}
		...
		if (plusMinus != null) {
			if (plusMinus < 0 || plusMinus > 2) {
				new DialogUtils(this)
						.showErrorDialog(R.string.plusMinusBetween0And2);
				return;
			}
		}
		final Set<Character> matches = Radicals
				.getKanjisWithRadicals(radicals.toCharArray());
		if(matches.size() > 2600 && (plusMinus == null || strokes == null))
		{
			new DialogUtils(this)
			.showErrorDialog("This radical has many result. Please enter stroke counts");
			return;
		}
	...
	private class KanjiMatchTask extends AbstractTask<Object, List<DictEntry>> {
		...
		public List<DictEntry> impl(Object... params) throws Exception {
			...
			final Set<Character> matches = Radicals
					.getKanjisWithRadicals(((String) params[0]).toCharArray());
			final List<DictEntry> entries = new ArrayList<DictEntry>();
			// filter the matches based on stroke count
			final LuceneSearch ls = new LuceneSearch(DictTypeEnum.Kanjidic,
					null, AedictApp.getConfig().isSorted());
			try {
				for (final Iterator<Character> kanjis = matches.iterator(); kanjis
						.hasNext();) {
					final char kanji = kanjis.next();
					final SearchQuery sq = SearchQuery.kanjiSearch(kanji,
					...
					if (++charsReportCountdown >= REPORT_EACH_XTH_CHAR) {
						...
						publish(new Progress(null, totalCharsProcessed,
								matches.size()));
					}
					if (isCancelled()) {
						return null;
		...
		protected void onSucceeded(List<DictEntry> result) {
			...
			finish();
			KanjiAnalyzeActivity.launch(DrawKanjiViewController.this,
					result, radical, mode, false);
		}
	}
	public static class KanjipadComparator implements Comparator<DictEntry> {
		...
		public int compare(DictEntry object1, DictEntry object2) {
			final int result = getStrokes(object1).compareTo(
					getStrokes(object2));
			if (result != 0) {
				return result;
			}
		...
		private Integer getStrokes(final DictEntry e) {
			return e instanceof KanjidicEntry ? ((KanjidicEntry) e).strokes
					: Integer.MAX_VALUE;
		}
	}
